...
- notifications are preserved unless another explicit policy applies;
...
- sent messages are anonymized according to the messaging policy below;
...
| Conversations | `participants[]` | Anonymize banned participant; preserve surviving user's conversation |
| Messages sent by user | `senderId` | Anonymize sender identity; preserve content |
| Messages received by user | `recipientId` | Preserve |
| Notifications received by user | `recipientId` | Delete |
| Notifications generated by user | `actorId` | Delete |
...
# 11. Direct Message Policy

Banning one user must not destroy the other participant's conversation history merely because the banned user is removed.

## Messages sent by the banned user

- preserve message content and timestamps;
- anonymize sender identity to the canonical deleted-user sentinel, e.g. `deleted_user`;
- do not retain the banned user's personal profile identity as the visible sender.

## Messages sent by the other participant to the banned user

Preserve them.

They are the other participant's historical messages.

## Conversations

For conversations involving the banned user:

- replace the banned user's participant identity with the deleted-user sentinel;
- remove the banned user's unread-count entry;
- preserve the conversation for the surviving participant;
- the conversation MUST remain readable.

If both participants are eventually deleted, a future maintenance process may garbage-collect the conversation, but that is outside this Ban implementation unless explicitly approved.

---

# 12. Notification Policy

### Notifications received by the banned user

Delete them.

### Notifications generated by the banned user

Delete them.

### Notifications generated by other users about content owned by the banned user

Preserve them unless another explicit policy requires removal.

Such notifications may reference deleted Tweets/Videos. The existing notification system must display a safe unavailable-content state.
...
The exact internal ordering of Tweets/Videos/Follows/Messages/Notifications may be optimized, but it MUST preserve the following rules:
...
- banned sender anonymized;
- other user's message history preserved;
- conversation remains readable.

### Notifications

- received/generated notifications cleaned according to policy;
- third-party notifications referencing deleted content remain safe.
