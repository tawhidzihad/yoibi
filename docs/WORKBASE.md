...
## Current Task
- Task ID: TASK-012
- Title: Authentication & User Profile Redesign — Signup/Login UX, Better Auth Google Sync, Canonical `users` Profile, JWT
...
- Summary of this task:
  - Signup UI redesigned (`frontend/src/features/auth/ui/SignupForm.js`): Full Name, Email Address, Telephone Number (optional), Password, Confirm Password, Age (>= 16, Zod-validated), Say About You / bio (optional, 280 max), Country (canonical ISO 3166-1 alpha-2 selector from `shared/constants/countries.js`), and the exact 7-item Community Values list (`shared/constants/communityValues.js`) with the required `communityValuesAgreed` checkbox. NO profile-picture upload on signup (`avatarUrl` starts null); React Hook Form + Zod field-level errors; legacy-inspired `GlowBorderCard` hover-border sections (Account / Security / About You / Community Values); Back to Home link; responsive 320–1440.
  - Signup flow: Better Auth `signUp.email` with `autoSignIn` (immediate login — email verification does not exist) → `/auth/me` hydration (server upserts the canonical profile from the verified JWT) → onboarding fields (country/age/phone/bio) persisted via authenticated `PATCH /users/me` → navigate to `/feed`.
  - Login UI redesigned (`frontend/src/features/auth/ui/LoginForm.js`): Email + Password, Login, Continue with Google, Back to Home. No forgot-password/reset/verification entry points.
  - `auth-client.js` repaired (`import { createAuthClient }` — missing `import` keyword was breaking the build).
  - Backend: single canonical `users` collection in `yoibi_database` keyed by unique `betterAuthUserId` (+ unique server-generated `handle`, email indexed); NO password/passwordHash fields on the app profile; profile upsert is idempotent (no duplicates on repeat Email/Google logins); Google avatar URL stored in `avatarUrl` from the server-side verified Better Auth identity (never browser-submitted); `role` is server-managed (`user` default; signup/OAuth/query cannot assign `admin`); all identity derived from the verified Better Auth JWT via jose/JWKS in `backend/src/middleware/auth.js`.
  - Contracts synced (`contracts/API-CONTRACT.md`, `contracts/openapi.yaml`): `/auth/me` response without `isEmailVerified` and with `bio/country/age/phone`; `PATCH /users/me` request body documented with country (ISO alpha-2), age (>=16), phone; field classification table (Better Auth-owned / user-submitted / server-generated / provider-derived / admin-managed); Better Auth identity mapping.
  - Tests: `frontend/tests/auth-simplified.test.js` updated for the new signup contract (fields incl. country/bio/communityValuesAgreed, immediate `/feed` navigation, no `rulesAgreed` legacy checkbox); `backend/tests/auth-architecture.test.js` covers role security, id spoofing, no-credentials-in-profile, removed features.
  - Quality gates: backend `npm test` 100% + `npm run lint` clean + `npm audit` 0 vulnerabilities; frontend `vitest` 37/37 + `npm run lint` clean + `npm run build` success (only `/login` and `/signup` auth routes exist; `/verify-email`, `/forgot-password`, `/reset-password` are 404 by absence).
