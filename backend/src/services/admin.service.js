...
async function banUser({ targetUserId, reason, confirmationHandle, adminUser, adminToken = null }) {
    ...
    try {
        ...
        // 6. Direct Message Anonymization Policy
        // Banned user's sent messages are preserved with senderId = 'deleted_user'
        const msgAnonymizeRes = await Message.updateMany(
            { senderId: targetUserId },
            { $set: { senderId: 'deleted_user' } }
        );
        deletedCounts.anonymizedMessages = msgAnonymizeRes.modifiedCount || 0;

        // In conversations, replace participant reference with 'deleted_user'
        await Conversation.updateMany(
            { participants: targetUserId },
            { $set: { 'participants.$': 'deleted_user' } }
        );

        // 7. Notification Purge
        // Delete notifications received by target and notifications generated by target
        ...
        // 8. Reports Policy: Moderation reports are PRESERVED and never deleted
    } catch (phaseCError) {
